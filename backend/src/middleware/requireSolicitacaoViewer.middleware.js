...
async function requireSolicitacaoViewer(req, res, next) {
  if (!req.user) {
  ...
  try {
    ...
    if (!permitido) {
      return res.status(403).json({ mensagem: 'Acesso negado à solicitação de colaborador.' });
    }
    return next();
  } catch (err) {
...
module.exports = requireSolicitacaoViewer;
